...
class Test_LSF_Status_Checker(unittest.TestCase):
    ...
    def test___get_status___bjobs_fails_three_times_but_says_DONE_in_the_fourth___job_status_is_success(self,
        ...
        lsf_status_checker = StatusChecker(123, "dummy", wait_between_tries=0.001, max_status_checks=4)
        actual = lsf_status_checker.get_status()
        expected = "success"
        self.assertEqual(actual, expected)
    ...
    @patch.object(StatusChecker, StatusChecker._get_lines_of_log_file.__name__,
    ...
    @patch.object(OSLayer, OSLayer.run_process.__name__, side_effect=BjobsError)
    @patch.object(StatusChecker, StatusChecker._get_lines_of_log_file.__name__, side_effect=FileNotFoundError)
    ...
    @patch.object(OSLayer, OSLayer.run_process.__name__, side_effect=BjobsError)
    @patch.object(StatusChecker, StatusChecker._get_lines_of_log_file.__name__, return_value=["...", "..."])
    ...
    @patch.object(OSLayer, OSLayer.run_process.__name__, side_effect=BjobsError)
    @patch.object(StatusChecker, StatusChecker._get_lines_of_log_file.__name__, return_value=["Successfully completed.", ""])
    ...
    def test____get_lines_of_log_file(self):
        lsf_status_checker = StatusChecker(123, "test_file.txt", wait_between_tries=0.001, max_status_checks=4)
